...
const PRESETS = [
  {
    ...
    getKeyframes(coordinates, cw, ch, duration, scene) {
      ...
      const delaySec = (scene && typeof scene.highlightDelay === 'number') ? scene.highlightDelay : 0.5;
      const delayT = delaySec / (duration || 4);
      const holdT = Math.min(0.95, zoomT + delayT);
      return [
