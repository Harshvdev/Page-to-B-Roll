...
function getClampedZoom(scene, coordinates, cw, ch, defaultZoom = 1.6) {
  let zoom = (scene && typeof scene.zoomLevel === 'number') ? scene.zoomLevel : defaultZoom;
  if (coordinates && coordinates.width > 0 && coordinates.height > 0) {
    const pad = 0.9;
    const maxFitScale = Math.min(cw / coordinates.width, ch / coordinates.height) * pad;
    if (zoom > maxFitScale) {
      zoom = maxFitScale;
    }
  }
...
const PRESETS = [
  {
    ...
    getKeyframes(coordinates, cw, ch, duration, scene) {
      const zoomLevel = getClampedZoom(scene, coordinates, cw, ch, 1.6);
  ...
  },
  {
    ...
    proOnly: false,
    getKeyframes(coordinates, cw, ch, duration, scene) {
      const zoomLevel = getClampedZoom(scene, coordinates, cw, ch, 1.5);
      const zoomDuration = (scene && typeof scene.zoomDuration === 'number') ? scene.zoomDuration : 1.5;
      const startDelay = (scene && typeof scene.startDelay === 'number') ? scene.startDelay : 0.0;
      const highlightDelay = (scene && typeof scene.highlightDelay === 'number') ? scene.highlightDelay : 0.5;
      const d = duration || 4;
  ...
  },
  {
    ...
    proOnly: false,
    getKeyframes(coordinates, cw, ch, duration, scene) {
      const zoomLevel = getClampedZoom(scene, coordinates, cw, ch, 1.4);
      const zoomDuration = (scene && typeof scene.zoomDuration === 'number') ? scene.zoomDuration : 1.5;
      const startDelay = (scene && typeof scene.startDelay === 'number') ? scene.startDelay : 0.0;
      const highlightDelay = (scene && typeof scene.highlightDelay === 'number') ? scene.highlightDelay : 0.5;
      const d = duration || 4;
  ...
  },
  {
    id: PRESET.UNDERLINE,
    name: 'Underline',
    description: 'Zoom to target, sleek underline draws under selection',
    proOnly: false,
    getKeyframes(coordinates, cw, ch, duration, scene) {
      const zoomLevel = getClampedZoom(scene, coordinates, cw, ch, 1.6);
      const zoomDuration = (scene && typeof scene.zoomDuration === 'number') ? scene.zoomDuration : 1.5;
      ...
      return [
        ...
        { time: startDelayT, scale: 1.0, overlayType: 'none', overlayProgress: 0 },
        { time: zoomEndT, scale: zoomLevel, overlayType: 'none', overlayProgress: 0 },
        { time: holdT, scale: zoomLevel, overlayType: 'underline', overlayProgress: 0 },
        { time: midpoint, scale: zoomLevel, overlayType: 'underline', overlayProgress: 1 },
        { time: 1, scale: zoomLevel, overlayType: 'underline', overlayProgress: 1 },
      ];
    },
  },
  {
  ...
  },
  {
    ...
    getKeyframes(coordinates, cw, ch, duration, scene) {
      ...
      const startDelayT = Math.min(0.8, startDelay / d);
      return [
        { time: 0, targetX: targetCenterX, targetY: ch / 2, scale: 1.0, overlayType: 'none', overlayProgress: 0 },
        { time: startDelayT, targetX: targetCenterX, targetY: ch / 2, scale: 1.0, overlayType: 'none', overlayProgress: 0 },
        { time: 1, targetX: targetCenterX, targetY: targetCenterY, scale: 1.0, overlayType: 'none', overlayProgress: 1 },
      ];
    },
  },
  {
  ...
  },
  {
    ...
    proOnly: true,
    getKeyframes(coordinates, cw, ch, duration, scene) {
      const zoomLevel = getClampedZoom(scene, coordinates, cw, ch, 1.2);
      const zoomDuration = (scene && typeof scene.zoomDuration === 'number') ? scene.zoomDuration : 1.5;
      const startDelay = (scene && typeof scene.startDelay === 'number') ? scene.startDelay : 0.0;
      const highlightDelay = (scene && typeof scene.highlightDelay === 'number') ? scene.highlightDelay : 0.5;
      const d = duration || 4;
  ...
  },
  {
    ...
    getKeyframes(coordinates, cw, ch, duration, scene) {
      ...
      const startDelayT = Math.min(0.8, startDelay / d);
      return [
        { time: 0, targetX: targetCenterX, targetY: ch / 2 - startY, scale: 1.0, overlayType: 'none', overlayProgress: 0 },
        { time: startDelayT, targetX: targetCenterX, targetY: ch / 2 - startY, scale: 1.0, overlayType: 'none', overlayProgress: 0 },
        { time: 1, targetX: targetCenterX, targetY: ch / 2 - endY, scale: 1.0, overlayType: 'none', overlayProgress: 1 },
      ];
    },
  },
];
