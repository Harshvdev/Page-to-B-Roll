...
export function generateSrtContent(scenes) {
  ...
  for (let i = 0; i < scenes.length; i++) {
    const start = time;
    const pauseDuration = scenes[i].pauseDuration !== undefined ? scenes[i].pauseDuration : 1.0;
    const end = time + scenes[i].duration + pauseDuration;
...
export function getSupportedMimeType(requestedFormat) {
  const mp4Types = [
...
export async function exportGif(frames, canvasWidth, canvasHeight, fps, onProgress) {
...
class GIFEncoder {
  ...
  quantizeColors(pixels) {
    ...
    // Sample pixels if image is large to keep analysis fast (aim for ~20,000 sampled pixels)
