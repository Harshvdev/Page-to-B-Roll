export async function stitchStrips(strips, pageWidth, pageHeight, devicePixelRatio, actualOffsets, offsetLeft) {
  const w = Math.round(pageWidth * devicePixelRatio);
  const h = Math.round(pageHeight * devicePixelRatio);
  console.log('[capture] stitchStrips: canvas=' + w + 'x' + h + ' strips=' + strips.length + ' offsetLeft=' + offsetLeft);
  ...
  const offsets = (Array.isArray(actualOffsets) && actualOffsets.length === bitmaps.length)
    ? actualOffsets
  ...
  for (let i = 0; i < bitmaps.length; i++) {
    ...
    const xOff = offsetLeft !== undefined ? Math.round(offsetLeft * devicePixelRatio) : Math.max(0, Math.round((bitmap.width - w) / 2));
    ctx.drawImage(bitmap, -xOff, yOff);
    console.log('[capture] Strip ' + i + ': bitmap=' + bitmap.width + 'x' + bitmap.height + ' xOff=' + xOff + ' yOff=' + yOff);
    bitmap.close();
  }
