...
export function interpolateKeyframes(keyframes, t, scene) {
  ...
  for (const key of allKeys) {
    ...
    if (a[key] !== undefined && b[key] !== undefined) {
      ...
      result[key] = a[key] !== undefined ? a[key] : b[key];
    }
  }
  return result;
}
...
export function computeTransform(coordinates, keyframe, canvasWidth, canvasHeight) {
  ...
  const pageWidth = coordinates ? (coordinates.pageWidth || canvasWidth) : canvasWidth;
  const pageHeight = coordinates ? (coordinates.pageHeight || canvasHeight) : canvasHeight;

  if (keyframe.x !== undefined && keyframe.y !== undefined) {
    translateX = keyframe.x;
    translateY = keyframe.y;
  } else if (keyframe.targetX !== undefined && keyframe.targetY !== undefined) {
    const canvasCenterX = canvasWidth / 2;
    const canvasCenterY = canvasHeight / 2;
    translateX = canvasCenterX - keyframe.targetX * scale;
    translateY = canvasCenterY - keyframe.targetY * scale;
  } else {
    const targetCenterX = coordinates ? (coordinates.x + coordinates.width / 2) : canvasWidth / 2;
    const targetCenterY = coordinates ? (coordinates.y + coordinates.height / 2) : canvasHeight / 2;
    const canvasCenterX = canvasWidth / 2;
    const canvasCenterY = canvasHeight / 2;
