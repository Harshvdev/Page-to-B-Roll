...
export function renderFrame(ctx, imageBitmap, scene, frameIndex, totalFrames, brandKit, prevScene, cachedDims) {
  ...
  const keyframe = interpolateKeyframes(preset.getKeyframes(scaledCoords, width, height, scene.duration, scene), t);
  ...
  if (prevScene && transition !== 'cut' && frameIndex < transitionFrames) {
    ...
    if (transition === 'dissolve') {
      ...
      const prevKeyframe = interpolateKeyframes(prevPreset.getKeyframes(prevScaledCoords, width, height, prevScene.duration, prevScene), 1.0);
