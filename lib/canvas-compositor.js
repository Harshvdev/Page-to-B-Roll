  ...
  if (keyframes && keyframes.length > 0) {
    if (!prevScene) {
      ...
      if (scene.presetId !== 'scroll_journey' && scene.presetId !== 'auto_scroll') {
        ...
        const startX = (width - pageWidth * startScale) / 2;
        const startY = 0;
        keyframes[0].scale = startScale;
        keyframes[0].x = startX;
        keyframes[0].y = startY;
        
        if (keyframes.length > 1 && keyframes[1].time < 0.9) {
          keyframes[1].scale = startScale;
          keyframes[1].x = startX;
          keyframes[1].y = startY;
        }
      }
    } else {
      ...
      keyframes[0].x = prevEndTransform.translateX;
      keyframes[0].y = prevEndTransform.translateY;
      
      if (keyframes.length > 1 && keyframes[1].time < 0.9) {
        keyframes[1].scale = prevEndTransform.scale;
        keyframes[1].x = prevEndTransform.translateX;
        keyframes[1].y = prevEndTransform.translateY;
      }
    }
  }
...
export function renderUnderlineOverlay(ctx, scene, progress, color, transform) {
  const scale = transform.scale;
  const lineRects = scene.coordinates.lineRects || [];
