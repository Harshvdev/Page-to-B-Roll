...
async function handleRenderVideo(payload) {
  ...
  try {
    isCancelled = false;
    startSilentAudio();
    const { scenes, strips, actualOffsets, brandKit, pageWidth, pageHeight, devicePixelRatio, offsetLeft, fps } = payload;
    ...
    imageBitmap = await stitchStrips(strips, pageWidth, pageHeight, devicePixelRatio || 1, actualOffsets, offsetLeft);
